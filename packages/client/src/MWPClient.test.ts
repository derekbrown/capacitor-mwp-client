...
const storageStoreSpy = jest.spyOn(
  ScopedPreferencesStorage.prototype,
  "storeObject",
);
const storageClearSpy = jest.spyOn(ScopedPreferencesStorage.prototype, "clear");
...
describe("MWPClient", () => {
  ...
  describe("handshake", () => {
    it("should perform a successful handshake", async () => {
      ...
      expect(storageStoreSpy).toHaveBeenCalledWith(
        "walletCapabilities",
        mockCapabilities,
      );
  ...
  describe("request", () => {
    beforeAll(() => {
      jest
        .spyOn(ScopedPreferencesStorage.prototype, "loadObject")
